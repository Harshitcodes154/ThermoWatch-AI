...
# ------------------------------------------------------------
# HUGGING FACE
# Replace these with your actual Hugging Face repositories
# ------------------------------------------------------------
...
# Put your FIRMS API key in Streamlit Secrets:
#
...
def fetch_live_firms():
    ...
    if df.empty:
        raise RuntimeError(
            "NASA FIRMS returned no live observations."
        )
...
def attribute_facilities(
    ...
    live["latitude"] = pd.to_numeric(
        live["latitude"],
        errors="coerce"
    )

    live["longitude"] = pd.to_numeric(
        live["longitude"],
        errors="coerce"
    )
    ...
    if facilities.empty:
        ...
        raise RuntimeError(
            "OSM facility dataset contains no valid coordinates."
        )
    ...
    result["facility_latitude"] = (
        nearest["latitude"].values
    )

    result["facility_longitude"] = (
        nearest["longitude"].values
    )
...
try:
    ...
    with st.spinner(
        "Fetching LIVE NASA FIRMS observations..."
    ):

        live = fetch_live_firms()

    with st.spinner(
        "Matching hotspots with nearest facilities..."
    ):
...
st.success(
    f"LIVE pipeline completed — "
    f"{len(predictions)} hotspots analyzed."
)
...
st.caption(
    "ThermoWatch AI | "
    "LIVE FIRMS + OSM + V5 Machine Learning"
)
