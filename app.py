...
HF_MODEL_URL = (
    "[https://huggingface.co/harshitcodes1544/sihharshit154/resolve/main/source_classifier_v5.joblib](https://huggingface.co/harshitcodes1544/sihharshit154/resolve/main/source_classifier_v5.joblib)"
)

HF_OSM_URL = (
    "[https://huggingface.co/datasets/harshitcodes1544/thermowatch-osm-data/resolve/main/osm_facilities.csv](https://huggingface.co/datasets/harshitcodes1544/thermowatch-osm-data/resolve/main/osm_facilities.csv)"
)

# Retrieve FIRMS API Key from Streamlit Secrets or OS Environment variable
FIRMS_API_KEY = st.secrets.get("FIRMS_API_KEY", os.environ.get("FIRMS_API_KEY", ""))

# NASA FIRMS VIIRS source
FIRMS_URL = "[https://firms.modaps.eosdis.nasa.gov/api/area/csv/](https://firms.modaps.eosdis.nasa.gov/api/area/csv/)"
...
def load_osm_facilities():
    response = requests.get(HF_OSM_URL, timeout=180)
    response.raise_for_status()
...
def fetch_live_firms(api_key: str):
    if not api_key:
        raise RuntimeError(
            "FIRMS_API_KEY is missing. Please set it in Streamlit Secrets or in the sidebar."
        )

    # VIIRS NOAA-20 NRT
    url = f"{FIRMS_URL}{api_key}/VIIRS_NOAA20_NRT/{INDIA_BBOX}/1"
    ...
    if df.empty:
        raise RuntimeError("NASA FIRMS returned no live observations for the specified bounding box.")
...
def run_v5_prediction(df, model):
    ...
    if missing:
        raise RuntimeError(f"Missing V5 features: {missing}")
...
user_api_key = st.sidebar.text_input(
    ...
    value=FIRMS_API_KEY,
    type="password",
    help="Get a free key at [https://firms.modaps.eosdis.nasa.gov/api/map_key/](https://firms.modaps.eosdis.nasa.gov/api/map_key/)"
)
...
st.sidebar.info("Live observations are fetched from NASA FIRMS VIIRS sensors over India.")
...
if not user_api_key:
    st.warning("Please provide a valid NASA FIRMS API Key in the sidebar to run the pipeline.")
    st.stop()
...
try:
    ...
    with st.spinner("Fetching LIVE NASA FIRMS observations..."):
        live = fetch_live_firms(user_api_key)
...
display_cols = [
    "hotspot_id", "facility_name", "latitude", "longitude", "frp",
    "predicted_source", "confidence", "distance_to_facility_km",
...
top_risk = predictions.sort_values("live_risk_score", ascending=False)[display_cols].head(20)
